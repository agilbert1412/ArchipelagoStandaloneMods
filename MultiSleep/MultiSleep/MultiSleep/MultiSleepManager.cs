...
namespace MultiSleep.MultiSleep
{
    public class MultiSleepManager
    {
        ...
        private static MultiSleepBehavior _currentMultiSleep;
        public static MultiSleepBehavior CurrentMultiSleep => _currentMultiSleep;
        private static int _multiSleepPrice = 0;

        public MultiSleepManager(IMonitor logger, IModHelper modHelper, Harmony harmony)
        {
            _logger = logger;
        ...
        public void InjectMultiSleepOption(int pricePerDay)
        {
            _multiSleepPrice = pricePerDay;
        ...
        public static bool PerformTouchAction_Sleep_Prefix(GameLocation __instance, string[] action, Vector2 playerStandingPosition)
        {
            ...
            {
                _logger.Log($"Failed in {nameof(PerformTouchAction_Sleep_Prefix)}:\n{ex}", LogLevel.Error);
                return MethodPrefix.RUN_ORIGINAL_METHOD;
            }
        }
        ...
        public static bool AnswerDialogueAction_SleepMany_Prefix(GameLocation __instance, string questionAndAnswer, string[] questionParams, ref bool __result)
        {
            ...
            {
                _logger.Log($"Failed in {nameof(AnswerDialogueAction_SleepMany_Prefix)}:\n{ex}", LogLevel.Error);
                return MethodPrefix.RUN_ORIGINAL_METHOD;
            }
        }
